 ...
 */
const convertToUpperCase = (str) => str.toLocaleUpperCase();
...
const getRectangleString = (width, height) => {
  ...
  return `${top}${frame.repeat(height - 2)}${bottom}`;
};
...
const encodeToRot13 = (str) => {
  const data = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz?! '.split('');
  const dataCode = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm?! '.split('');
  const result = [];
  str.split('').map((elem) => {
    const indx = data.indexOf(elem);
    return result.push(dataCode[indx]);
  });
